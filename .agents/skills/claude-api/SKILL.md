...
This is a TypeScript-only local adaptation of the official Anthropic skill
(https://github.com/anthropics/skills/tree/main/skills/claude-api).
Unused language reference folders were removed for Waymark. Shared references
and the upstream license are retained. Reinstalling upstream may restore them.
...
**Never guess SDK usage.** Function names, class names, namespaces, method signatures, and import paths must come from explicit documentation - either the `typescript/` files in this skill or the official SDK repositories or documentation links listed in `shared/live-sources.md`. If the binding you need is not explicitly documented in the skill files, WebFetch the relevant SDK repo from `shared/live-sources.md` before writing code. Do not infer Ruby/Java/Go/PHP/C# APIs from cURL shapes or from another language's SDK.

**If WebFetch or repository access fails** (network restricted, timeouts, clone blocked): do not keep retrying - write code from the patterns and namespace/package tables in the `typescript/` file, run the compiler or interpreter on it, and iterate on the error output. For statically-typed SDKs (C#, Java, Go) a compile-fix loop against local errors reaches working code faster than blocked network research.
...
Several common Claude API shapes changed in 2025-2026. If you recall a pattern from training, verify it against the `typescript/` files in this skill before writing - the rows below are the most frequent drift points:
...
The `typescript/` files in this skill are authoritative over recalled patterns.
...
| `upgrade` | No TypeScript major-version upgrade guide is bundled. Consult the official TypeScript SDK changelog via `shared/live-sources.md` and apply only changes needed for the requested scope. |
...
Waymark uses TypeScript and `@anthropic-ai/sdk`. Read `typescript/` references
and relevant `shared/` documents on demand. Other language folders are not bundled.
If another language or raw HTTP is explicitly requested, consult the official
sources in `shared/live-sources.md` instead of looking for removed local files.
...
See `typescript/claude-api/README.md` (Compaction section) for code examples. Full docs via WebFetch in `shared/live-sources.md`.
...
For placement patterns, architectural guidance, and the silent-invalidator audit checklist: read `shared/prompt-caching.md`. Language-specific syntax: `typescript/claude-api/README.md` (Prompt Caching section).
...
For this TypeScript project, read the relevant files based on what the user needs. Every `typescript/...` and `shared/...` path cited in this document is relative to this skill's base directory, and none of those files' content is included above - Read each one on demand before relying on what it covers.

The `typescript/claude-api/` directory contains the SDK overview, tool-use, streaming, batches, and files guides. Read only the relevant files. For missing examples, consult the official SDK sources in `shared/live-sources.md`.

The Quick Task Reference below uses the `typescript/claude-api/FILE.md` path notation for all languages.

### Quick Task Reference

**Single text classification/summarization/extraction/Q&A:**
-> Read only `typescript/claude-api/README.md` - **always read the README first** for any task (installation, quick start, common patterns, error handling)

**Chat UI or real-time response display:**
-> Read `typescript/claude-api/README.md` + `typescript/claude-api/streaming.md`

**Long-running conversations (may exceed context window):**
-> Read `typescript/claude-api/README.md` - see Compaction section
...
-> Read `typescript/claude-api/sdk-upgrade.md` (currently Python only; other SDKs have no bundled major-version guide yet - use that SDK's CHANGELOG via `shared/live-sources.md`)
...
-> Read `shared/prompt-caching.md` (prefix-stability design, breakpoint placement, anti-patterns that silently invalidate cache) + `typescript/claude-api/README.md` (Prompt Caching section)
...
-> Read `typescript/claude-api/README.md` + `shared/tool-use-concepts.md` (conceptual foundations: function calling, code execution, memory, structured outputs) + `typescript/claude-api/tool-use.md` (language-specific code examples: tool runner, manual loop, code execution, memory, structured outputs)
...
-> Read `typescript/claude-api/README.md` + `typescript/claude-api/batches.md`

**File uploads across multiple requests (same file without re-uploading):**
-> Read `typescript/claude-api/README.md` + `typescript/claude-api/files-api.md`
...
-> See the reading guide in the `## Managed Agents (Beta)` section above - it lists every `shared/managed-agents-*.md` file and the language-specific READMEs (`typescript/managed-agents/README.md`).
